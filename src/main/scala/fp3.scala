...
object fp3:
  ...
  def allEqual(xs: List[Int]): Boolean =
    ...
    else if (xs.head != xs.tail.head) then false
    else allEqual(xs.tail)
  ...
  def delete1[X](x: X, ys: List[X]): List[X] =
    ...
    if (ys == Nil) then Nil
    else if ys.head != x then ys.head :: delete1(x,ys.tail)
    else delete1(x,ys.tail)
  ...
  if(xs == Nil) then Nil
  else if (xs.tail == Nil) then xs
  else if (xs.head != xs.tail.head) then xs.head :: removeDupes1(xs.tail)
  else removeDupes1(xs.tail)
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    def removeD [X] (x:X, xs:List[X],i:Int) :List[(Int,X)] = {
      if xs == Nil then 
          if x == Nil then List() else (i,x) :: List()
      else if xs.head == x then removeD(x,xs.tail,i+1) else (i,x) :: removeD(xs.head,xs.tail,1)
      } 
      if(xs.length > 0 ) then removeD(xs.head,xs.tail,1)
      else List()
}
  ...
  def splitAt[X](n: Int, xs: List[X]): (List[X], List[X]) =
    ...
    if ( xs == Nil) then (Nil,Nil)
      ...
      val(lista,listb) = splitAt(n-1,xs.tail) 
      if (n> 0) then (xs.head::lista,listb) 
      else (lista,xs.head::listb)
  ...
  if (xs == Nil) then true
  else if member(xs.head, xs.tail) then false
